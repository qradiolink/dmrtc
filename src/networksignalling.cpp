    ...
    : QObject{parent}
{
    m_settings = settings;
    m_logger = logger;
    m_be = (QSysInfo::ByteOrder == QSysInfo::BigEndian) ? true : false;
}
...
{
    if (size < 5) {
        m_logger->log(Logger::LogLevelWarning, QString("Received invalid network message with size %1").arg(size));
        return false;
    }

    if (memcmp(message, "DMRT", 4U) != 0) {
        m_logger->log(Logger::LogLevelWarning, QString("Received network message with unknown signature"));
        return false;
    }
    ...
    if (opcode_validation != 0x40) {
        m_logger->log(Logger::LogLevelWarning, QString("Received network message with invalid opcode %1").arg(message[4U]));
        return false;
    }
...
{
    if (tgs.size() < 1) {
        m_logger->log(Logger::LogLevelWarning, QString("Cannot create subscription message with %1 talkgroups.").arg(tgs.size()));
        return false;
    }

    if (tgs.size() > 64) {
        m_logger->log(Logger::LogLevelWarning, QString("Only the first 64 of the requested %1 talkgroups will be subscribed.").arg(tgs.size()));
    }
...
{
    if (tgs.size() < 1) {
        m_logger->log(Logger::LogLevelWarning, QString("Cannot create unsubscription message with %1 talkgroups.").arg(tgs.size()));
        return false;
    }

    if (tgs.size() > 64) {
        m_logger->log(Logger::LogLevelWarning, QString("Only the first 64 of the requested %1 talkgroups will be unsubscribed.").arg(tgs.size()));
    }
