...
ChannelViewModel::ChannelViewModel(QObject* parent): QAbstractTableModel(parent)
{
    for (unsigned int i = 0; i < ROWS; i++) {
        for (unsigned int j = 0; j < COLS; j++) {
            m_colors[i][j] = "#FFFFFF";
            m_check_state[i][j] = Qt::Checked;
            m_state[i][j] = ChannelState::ChannelUnused;
        }
    }
}
...
QVariant ChannelViewModel::data(const QModelIndex& index, int role) const
{
    ...
    switch (role) {
        case Qt::DisplayRole: {
            ...
            QString text = QString("Usage: %1").arg(m_grid_data[row][col]);
            return text;
        }
        ...
        case Qt::FontRole: {
            ...
            if (m_state[row][col] == ChannelState::ChannelControl) {
                return boldFont;
            }

            if ((m_colors[row][col] == "#004d99") || (m_colors[row][col] == "#004dFF") || (m_colors[row][col] == "#029500")) {
                return boldFont;
            }
        ...
        case Qt::BackgroundRole: {
            ...
            return QBrush(QColor(m_colors[row][col]));
            break;
        }
        ...
        case Qt::DecorationRole: {
            if (m_check_state[index.row()][index.column()] == Qt::Unchecked) {
                return QIcon(":/res/user-busy.png");
            }

            if (m_state[index.row()][index.column()] == ChannelState::ChannelBusy) {
...
bool ChannelViewModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        m_grid_data[row][col] = value.toString();
        emit dataChanged(this->index(0, 0), this->index(ROWS - 1, COLS - 1));
        return true;
    }

    if (role == Qt::CheckStateRole) {
        m_check_state[row][col] = value.toInt();
        emit dataChanged(this->index(0, 0), this->index(ROWS - 1, COLS - 1));
...
void ChannelViewModel::setColor(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    m_colors[index.row()][index.column()] = value.toString();
    emit dataChanged(this->index(0, 0), this->index(ROWS - 1, COLS - 1));
}
...
void ChannelViewModel::setState(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    m_state[index.row()][index.column()] = value.toInt();
    emit dataChanged(this->index(0, 0), this->index(ROWS - 1, COLS - 1));
}
